# Does your product actually appear on these portals? — 19 September 2026

You asked me to browse for anything similar to what you sell. I read **all 49 live
IWK events** on Procurehere. Here is the honest answer.

## Almost nothing matches

The 49 live IWK events break down as: manhole cover replacement, sewer repair by
CIPP, desludging and sludge removal, minor civil works, grass cutting, aircond
servicing, diffuser replacement, SCADA and instrumentation, electronic security
systems, labour supply contracts, a genset radiator.

**Exactly one is even adjacent to your business:**

> `BSE/KT/04/09/26` — KERJA-KERJA RENOVATION PEJABAT BARU IWK TERENGGANU GONG BADAK
> Categories: **CIVIL WORKS, FURNITURE, GENERAL BUILDING MAINTENANCE, OFFICE
> EQUIPMENT, OFFICE SUPPLIES, RENOVATION**
> Closes 22/09/2026

And even that is a **renovation contract in Terengganu** — outside your free
delivery zone, and it wants a contractor, not a furniture supplier.

## What this means — the same lesson as MyProcurement

**This is the second time today the same pattern has appeared.** The portals are
real, free and open. Your product category is thin on all of them.

IWK is a sewerage company. Its spend is civil works and plant maintenance, not
workshop storage. Expecting tool cabinet tenders there was my assumption, and the
data does not support it.

## But two things are genuinely useful

**1. The category taxonomy includes FURNITURE, HARDWARE, OFFICE EQUIPMENT and
GENERAL SUPPLIES.** Those tags exist, which means such events do occur — just not
in this week's 49. Registered suppliers get **invited** to tenders in their
categories rather than having to spot them. That is the real mechanism: you are not
browsing for work, you are on the invitation list.

**2. Browsing is free and needs no account.** Checking IWK and FELCRA weekly costs
fifteen minutes and tells you whether your categories ever come up. **Do that for
three or four weeks before paying for anything.**

## Recommendation, revised again

**Do the free registrations. Do not pay for Procurehere yet.**

The subscription shows as **145.80** on the checkout page (single plan, labelled
"Unlimited Buyer Plan"). That is not much — but on one week's evidence there is
nothing in your categories to bid on, and the free public events let you test that
for a month at zero cost.

**The strongest reason to register anywhere remains the invitation list, not the
public feed.** Air Selangor, IWK and Prasarana all work that way, and all three are
free.

---

# CORRECTION — IWK's supplier categories do include your product

I wrote above that the category match was thin. **That was right about this week's
49 live events and wrong about the registration itself.**

IWK's Contractor & Supplier registration page lists the **Supplier** categories
explicitly, and these are verbatim from their page:

| Category | Their own description |
|---|---|
| **Hardware** | **Tools / power tools / bolts & nuts** |
| **Furniture / Building Maintenance** | — |
| **Office Equipment** | — |
| **Office Supplies** | — |
| **Safety equipment** | Tripod / gas detector |
| **General supplies** | — |
| **Laboratory** | Chemicals / equipment / analysis |
| **Renovation** | — |

**Hardware (Tools) and Furniture are both explicit supplier categories.** Tool
cabinets and CNC storage register under Hardware; lockers, workbenches and
cabinets under Furniture / Building Maintenance.

And the page states the mechanism in one line:

> *"Only registered Contractor/Supplier will be listed in the tender invitation
> list for tender in any Indah Water Projects."*

**So the public feed was never the point.** You register into a category and get
invited. Browsing 49 live events told me nothing about how much Hardware and
Furniture IWK buys in a year — only that none of it was open this week.

## Also important: CIDB and SPAN do not apply to you

Their requirements page tags SPAN and CIDB as **"- Contractors"**, and the
Contractor section says registration with CIDB and SPAN is compulsory *"for all
construction works involving Capital Works and Refurbishment."*

**You are registering under Supplier (Section B), not Contractor (Section A).**
Nothing in the Supplier path requires CIDB, SPAN or MOF.

**Register with IWK. It is free, it takes you off the public feed and onto the
invitation list, and two of their supplier categories are literally your product.**
