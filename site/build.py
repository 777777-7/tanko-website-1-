...
# ---------- Critical CSS ---------------------------------------------------
# Inlined into <head> so first paint doesn't wait for the full site.css.
# Covers :root palette + font + baseline body + header + home-v4 hero +
# generic category/family stage styling. site.css is then loaded async via
# rel="preload" as="style" onload="this.rel='stylesheet'" so it's not
# render-blocking. Small enough (~3.5 KB) to keep the head-inline overhead
# negligible on 300-500 byte gzip cost.
CRITICAL_CSS = (
    "*,*::before,*::after{box-sizing:border-box}"
    "html,body{margin:0;padding:0}"
    ":root{--bg:#0a0d13;--bg-alt:#0e131c;--surface:#131926;--surface-2:#1a2130;"
    "--line:#26303f;--line-soft:#1d2531;--ink:#e9eef6;--ink-muted:#93a2b7;"
    "--accent:#ff3b47;--steel:#c8d3e2;--max:1240px;"
    "--font:\"Public Sans\",-apple-system,BlinkMacSystemFont,\"Segoe UI\",Roboto,sans-serif;"
    "--serif:\"Libre Bodoni\",\"Bodoni Moda\",Georgia,serif;"
    ...
    ".site-header{background:rgba(10,13,19,.9);border-bottom:1px solid var(--line-soft);"
    "position:sticky;top:0;z-index:100;backdrop-filter:blur(8px)}"
    ".site-header .container{display:flex;align-items:center;justify-content:space-between;gap:1rem;padding:.4rem 1.25rem}"
    ".brand{display:flex;align-items:center;gap:.6rem;font-weight:700;color:#fff}"
    ".brand-logo-primax{height:46px;width:auto}"
    ...
    ".btn{display:inline-block;padding:.55rem 1rem;background:var(--accent);color:#fff;"
    "border-radius:6px;font-weight:600;font-size:.9rem;border:0;cursor:pointer}"
    "@media(max-width:979px){.nav{display:none!important}}"
    "@media(min-width:980px){.mobnav{display:none}}"
    ".mobnav summary{cursor:pointer;padding:.5rem .8rem;border:1px solid var(--line);"
    "border-radius:6px;color:var(--steel);list-style:none;font-size:1.5rem}"
    ".mobnav summary::-webkit-details-marker{display:none}"
    # ---------------- home-v4 hero (paper editorial) ----------------
    "body.home-v4{--paper:#f3efe8;--paper-2:#eae5db;--rule:#d7d1c4;--rule-2:#b8b1a2;"
    "--accent:#c62828;--accent-ink:#fff;--ink:#0d1117;--ink-2:#2a2f36;--ink-muted:#5c6470;"
    "--body:\"Public Sans\",-apple-system,BlinkMacSystemFont,\"Segoe UI\",Roboto,sans-serif;"
    "--display:\"Libre Bodoni\",\"Bodoni Moda\",Georgia,serif;"
    "--mono:\"JetBrains Mono\",ui-monospace,SFMono-Regular,Menlo,Consolas,monospace;"
    "background:var(--paper);color:var(--ink);font-family:var(--body)}"
    "body.home-v4 a{color:inherit}"
    "body.home-v4 .site-header{background:var(--paper);border-bottom:1px solid var(--rule);backdrop-filter:none}"
    "body.home-v4 .brand{color:var(--ink)}"
    "body.home-v4 .brand img{height:64px;width:auto;object-fit:contain}"
    "body.home-v4 .brand-text{color:var(--ink)}"
    "body.home-v4 .v4-hero{position:relative;background:var(--paper);border-bottom:1px solid var(--rule);overflow:hidden}"
    "body.home-v4 .v4-hero-track{position:relative}"
    "body.home-v4 .v4-slide{display:none}"
    "body.home-v4 .v4-slide.is-active{display:block}"
    "body.home-v4 .v4-hero-inner{max-width:1240px;margin:0 auto;padding:56px 24px 64px;"
    "display:grid;grid-template-columns:1fr;gap:40px;align-items:center}"
    "@media(min-width:900px){body.home-v4 .v4-hero-inner{grid-template-columns:1.05fr .95fr;gap:64px;padding:96px 24px}"
    "body.home-v4 .brand img{height:76px}}"
    "body.home-v4 .v4-hero-copy{max-width:560px}"
    "body.home-v4 .v4-hero-tanko-logo{height:28px;width:auto;margin-bottom:20px}"
    "body.home-v4 .v4-eyebrow{display:flex;align-items:center;font-family:var(--mono);"
    "font-size:11px;letter-spacing:.14em;color:var(--ink-muted);text-transform:uppercase;"
    "margin:0 0 24px;font-weight:500}"
    "body.home-v4 .v4-rule{display:inline-block;width:34px;height:1px;background:var(--accent);"
    "margin-right:12px;vertical-align:middle}"
    "body.home-v4 .v4-hero-h{font-family:var(--display);font-weight:500;color:var(--ink);"
    "font-size:clamp(44px,6.4vw,84px);line-height:.96;letter-spacing:-.022em;margin:0 0 24px;text-wrap:balance}"
    "body.home-v4 .v4-hero-sub{font-family:var(--body);font-size:17px;line-height:1.55;"
    "color:var(--ink-2);max-width:50ch;margin:0 0 32px}"
    "body.home-v4 .v4-hero-actions{display:flex;gap:12px;flex-wrap:wrap}"
    "body.home-v4 .v4-btn{display:inline-flex;align-items:center;justify-content:center;"
    "padding:14px 22px;border-radius:3px;font-family:var(--body);font-weight:600;font-size:14px;"
    "letter-spacing:.02em;border:1.5px solid transparent;text-decoration:none;cursor:pointer}"
    "body.home-v4 .v4-btn-primary{background:var(--accent);color:var(--accent-ink)}"
    "body.home-v4 .v4-btn-ghost{background:transparent;color:var(--ink);border-color:var(--ink)}"
    "body.home-v4 .v4-hero-photo{position:relative;margin:0;padding:0;background:var(--paper);"
    "border:1px solid var(--rule);aspect-ratio:4/5;display:flex;align-items:center;"
    "justify-content:center;overflow:hidden}"
    "body.home-v4 .v4-hero-photo img{max-width:92%;max-height:82%;object-fit:contain;"
    "mix-blend-mode:multiply;filter:drop-shadow(0 20px 40px rgba(0,0,0,.16))}"
)
env.globals["critical_css"] = CRITICAL_CSS
